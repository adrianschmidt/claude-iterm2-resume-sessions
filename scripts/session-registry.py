#!/usr/bin/env python3
"""Record interactive Claude Code sessions so they can be resumed after a reboot.

SessionStart hook: upsert a record for the session.
SessionEnd hook:   delete the record on a deliberate exit, otherwise mark how
                   it ended. A reboot/SIGHUP ends with reason "other", so those
                   records survive and are what the boot script resumes.
    ...
    record.update(
        {
            ...
            "iterm_session_id": os.environ.get("ITERM_SESSION_ID", "").split(":")[-1] or None,
...
def on_end(payload):
    path = record_path(payload["session_id"])
